...
import { MapPin, MapPinned, ShieldCheck } from "lucide-react";
        ...
        {!isPending && data?.data && data?.data.length > 0 && (
          <div className="liquid-glass-effect rounded-xl p-4">
            ...
            <p className="text-[16px]">
              {data?.data[0].town}, {data?.data[0].region} Region
            </p>
            ...
            <div className="flex flex-col text-xs mt-4 ">
              {data.data[0].location.lat === 0 ||
                (data.data[0].location.lng === 0 && (
                  <p className="text-xs text-red-400 mb-1">
                    Location is not set
                  </p>
                ))}
              <Link
                href={"/pharmacy/update-location"}
                className="border py-2 rounded px-4 border-gray-400 inline-flex gap-2 text-sm items-center w-full cursor-pointer trans hover:border-neutral-800 text-neutral-700"
              >
                <MapPinned size={18} />
                {data.data[0].location.lat === 0 ||
                data.data[0].location.lng === 0 ? (
            ...
            {!data.data[0].isVerified && (
              <Link
                href={"/pharmacy/verify-account"}
                className="border py-2 rounded px-4 border-gray-400 mt-4 inline-flex gap-2 text-sm items-center w-full cursor-pointer trans hover:border-neutral-800 text-neutral-700"
              >
                <ShieldCheck size={18} />
                Verify Account
              </Link>
