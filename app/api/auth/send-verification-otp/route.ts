...
import { generateOtpExpiry } from "@/libs/generateOtpExpiry";
...
export async function POST(req: Request) {
  ...
  const time = generateOtpExpiry(10)
console.log(otp)
  await db.update(pharmacyTable)
  .set({verificationOTP: otp, verificationOTPExpired: time})
  .where(and(...conditions))

    return NextResponse.json(
      {
        message: `OTP sent to email ${pharmacy[0].email}.`,
        data: [],
        success: true,
      },
      { status: 200 }
