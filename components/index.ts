
//Components index file to export all components
export { default as Footer } from './Footer'
export { default as PrivateNavbar } from './PharmacyNavbar'
export { default as PublicNavbar } from './PublicNavbar'
export { default as Hero } from './Hero'
export { default as HeroCTA } from './HeroCTA'
export { default as Header } from './Header'
export { default as Logo } from './Logo'
export { default as Navlinks } from './Navlinks'
export { default as User } from './User'
export { default as Hamburger } from './Hamburger'
export { default as Language } from './Language'
export { default as Searchbar } from './searchDrugs'
export { default as PreferedTown } from './PreferedTown'
export { default as PharmacyGrid } from './PharmacyGrid'
export { default as PharmacyCard } from './PharmacyCard'
export { default as Back } from './Back'
export { default as WeekDays } from './WeekDays'
export { default as ChangeCity } from './ChangeCity'
export { default as NoPharmacy } from './NoPharmacy'
export { default as Sidebar } from './Sidebar'
export { default as Loading } from './Loading'
export { default as PharmacyHero } from './PharmacyHero'
export { default as DrugDetailsCard } from './DrugDetailsCard'
export { default as NoData } from './NoData'
export { default as ErrorFetching } from './ErrorFetching'
export { default as MyModal } from './MyModal'
export { default as Title } from './Title'
export { default as Spiner } from './Spiner'
export { default as PharmacyDailyProgram } from './PharmacyDailyProgram'
export { default as ManualClosePharmcy } from './ManualClosePharmacy'
export { default as SearchPharmacy } from './SearchPharmacy'
export { default as PharmacyStatus } from './PharmacyStatus'
export { default as PharmacyOnCallStatus } from './PharmacyOnCallStatus'
export { default as PharmacyTotalDrugs } from './PharmacyTotalDrugs'
export { default as PharmacyHeroCTA } from './PharmacyHeroCTA'


// ui
// export { default as CurrentDate } from './ui/currentDate'
export { default as HeroPharmacyName } from './ui/HeroPharmacyName'
export { default as HeroTotalOrders } from './ui/HeroTotalOrders'